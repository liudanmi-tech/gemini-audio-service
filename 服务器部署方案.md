# 服务器部署方案 - 使用现有阿里云服务器

## 🎯 答案：可以使用同一台服务器！

**结论**：你可以在**同一台阿里云服务器**（47.79.254.213）上运行 FastAPI 后端服务。

**优势**：
- ✅ 不需要额外服务器，节省成本
- ✅ 网络延迟更低（后端和 Gemini 代理在同一台服务器）
- ✅ 配置更简单
- ✅ 维护更方便

---

## 📋 当前架构

### 现状：
```
iOS 客户端 → 阿里云服务器 (47.79.254.213)
                ↓
            Nginx (反向代理)
                ↓
            Google Gemini API
```

### 目标架构：
```
iOS 客户端 → 阿里云服务器 (47.79.254.213)
                ↓
            Nginx (API Gateway)
                ├─→ FastAPI 后端服务 (端口 8001)
                └─→ Google Gemini API (反向代理)
```

---

## 🏗️ 部署方案

### 方案概述

在同一台阿里云服务器上：
1. **运行 FastAPI 后端服务**（端口 8001）
2. **Nginx 同时作为**：
   - API Gateway：接收 iOS 客户端请求，转发到 FastAPI
   - 反向代理：转发 Gemini API 请求

---

## 📝 详细部署步骤

### 步骤 1: 在服务器上部署 FastAPI 后端

#### 1.1 连接到服务器

```bash
ssh admin@47.79.254.213
```

#### 1.2 创建项目目录

```bash
# 创建项目目录
mkdir -p /opt/work-survival-backend
cd /opt/work-survival-backend
```

#### 1.3 上传项目文件

将以下文件上传到服务器：
- `main.py`
- `requirements.txt`
- `.env`（包含 GEMINI_API_KEY）

**上传方法**：
```bash
# 在本地 Mac 上执行
scp main.py requirements.txt admin@47.79.254.213:/opt/work-survival-backend/
scp .env admin@47.79.254.213:/opt/work-survival-backend/
```

#### 1.4 安装 Python 和依赖

```bash
# 在服务器上执行
# 安装 Python 3.14（如果还没有）
sudo apt update
sudo apt install python3.14 python3.14-venv python3-pip -y

# 创建虚拟环境
python3.14 -m venv venv
source venv/bin/activate

# 安装依赖
pip install -r requirements.txt
```

#### 1.5 配置环境变量

```bash
# 编辑 .env 文件
nano .env
```

确保 `.env` 文件包含：
```
GEMINI_API_KEY=AIzaSyCiOOgxgMkTuqw6sXTT08WbD7R6kMK-k08
PROXY_URL=http://47.79.254.213/secret-channel
USE_PROXY=true
```

#### 1.6 测试运行

```bash
# 激活虚拟环境
source venv/bin/activate

# 运行服务（测试）
python3 main.py
```

如果运行成功，应该能看到服务启动在 `http://0.0.0.0:8001`

---

### 步骤 2: 配置 Nginx 作为 API Gateway

#### 2.1 编辑 Nginx 配置

```bash
# 在服务器上执行
sudo nano /etc/nginx/sites-available/default
```

#### 2.2 添加 FastAPI 后端代理配置

在现有的 Nginx 配置中，**添加**以下配置（不要删除现有的 `/secret-channel` 配置）：

```nginx
# FastAPI 后端服务代理（新增）
location /api/ {
    proxy_pass http://127.0.0.1:8001/;
    proxy_set_header Host $host;
    proxy_set_header X-Real-IP $remote_addr;
    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
    proxy_set_header X-Forwarded-Proto $scheme;
    
    # 支持大文件上传
    client_max_body_size 100M;
    client_body_buffer_size 128k;
    
    # 超时设置
    proxy_connect_timeout 600s;
    proxy_send_timeout 600s;
    proxy_read_timeout 600s;
    
    # 禁用缓冲（用于文件上传）
    proxy_buffering off;
    proxy_request_buffering off;
}

# Gemini API 反向代理（保持现有配置）
location /secret-channel {
    rewrite ^/secret-channel(.*) $1 break;
    proxy_pass https://generativelanguage.googleapis.com;
    proxy_set_header Host generativelanguage.googleapis.com;
    proxy_ssl_server_name on;
    proxy_ssl_verify off;
    client_max_body_size 100M;
    proxy_read_timeout 600s;
}

location /upload {
    proxy_pass https://generativelanguage.googleapis.com;
    proxy_set_header Host generativelanguage.googleapis.com;
    proxy_ssl_server_name on;
    proxy_ssl_verify off;
    client_max_body_size 100M;
    proxy_read_timeout 600s;
}
```

#### 2.3 测试 Nginx 配置

```bash
# 测试配置是否正确
sudo nginx -t
```

如果显示 `syntax is ok` 和 `test is successful`，说明配置正确。

#### 2.4 重新加载 Nginx

```bash
# 重新加载 Nginx 配置
sudo systemctl reload nginx
```

---

### 步骤 3: 配置 FastAPI 服务自动启动

#### 3.1 创建 systemd 服务文件

```bash
sudo nano /etc/systemd/system/work-survival-backend.service
```

#### 3.2 添加服务配置

```ini
[Unit]
Description=Work Survival Guide Backend API
After=network.target

[Service]
Type=simple
User=admin
WorkingDirectory=/opt/work-survival-backend
Environment="PATH=/opt/work-survival-backend/venv/bin"
ExecStart=/opt/work-survival-backend/venv/bin/python3 main.py
Restart=always
RestartSec=10

[Install]
WantedBy=multi-user.target
```

#### 3.3 启动服务

```bash
# 重新加载 systemd
sudo systemctl daemon-reload

# 启动服务
sudo systemctl start work-survival-backend

# 设置开机自启
sudo systemctl enable work-survival-backend

# 查看服务状态
sudo systemctl status work-survival-backend
```

---

### 步骤 4: 配置防火墙

```bash
# 确保 80 端口（HTTP）和 443 端口（HTTPS，如果配置了 SSL）已开放
sudo ufw allow 80/tcp
sudo ufw allow 443/tcp

# 检查防火墙状态
sudo ufw status
```

---

## 🔧 修改 iOS 客户端 API 地址

### 在 NetworkManager.swift 中修改

1. **打开 `Services/NetworkManager.swift`**

2. **找到 `baseURL` 这一行**（大约第 17 行）

3. **修改为**：
   ```swift
   private let baseURL = "http://47.79.254.213/api/v1"
   ```
   或者如果配置了 HTTPS：
   ```swift
   private let baseURL = "https://47.79.254.213/api/v1"
   ```

4. **保存文件**

---

## 📊 最终架构

```
iOS 客户端
    ↓
    http://47.79.254.213/api/v1/...
    ↓
阿里云服务器 (47.79.254.213)
    ↓
Nginx (端口 80)
    ├─→ /api/ → FastAPI 后端 (127.0.0.1:8001)
    │       ↓
    │   FastAPI 调用 Gemini API
    │       ↓
    └─→ /secret-channel → Google Gemini API (反向代理)
```

---

## ✅ 验证部署

### 1. 测试后端服务

在服务器上：
```bash
# 检查服务是否运行
sudo systemctl status work-survival-backend

# 查看日志
sudo journalctl -u work-survival-backend -f
```

### 2. 测试 API 接口

在本地 Mac 上：
```bash
# 测试健康检查
curl http://47.79.254.213/api/v1/health

# 应该返回：
# {"message":"音频分析服务正在运行","status":"ok"}
```

### 3. 测试 iOS 客户端

在 iOS 客户端中：
- 修改 `NetworkManager.swift` 中的 `baseURL` 为 `http://47.79.254.213/api/v1`
- 运行项目
- 测试上传音频功能

---

## 🔒 安全建议（可选）

### 配置 HTTPS（推荐）

1. **安装 Certbot**：
   ```bash
   sudo apt install certbot python3-certbot-nginx -y
   ```

2. **申请 SSL 证书**：
   ```bash
   sudo certbot --nginx -d your-domain.com
   ```

3. **修改 iOS 客户端**：
   ```swift
   private let baseURL = "https://your-domain.com/api/v1"
   ```

---

## 📝 总结

### ✅ 可以使用同一台服务器

**优势**：
- 节省成本（不需要额外服务器）
- 配置简单（Nginx 同时处理两个功能）
- 性能更好（后端和代理在同一台服务器）

**需要做的**：
1. 在服务器上部署 FastAPI 后端
2. 配置 Nginx 同时作为 API Gateway 和反向代理
3. 修改 iOS 客户端的 API 地址

**不需要做的**：
- ❌ 不需要购买新服务器
- ❌ 不需要搭建新的 Nginx

---

## 🆘 如果遇到问题

### 问题 1: 端口冲突

如果 8001 端口被占用，可以修改：
- `main.py` 中的端口号
- Nginx 配置中的 `proxy_pass` 地址

### 问题 2: 服务无法启动

检查：
- Python 版本是否正确
- 依赖是否安装完整
- `.env` 文件是否正确配置

### 问题 3: Nginx 配置错误

检查：
- `sudo nginx -t` 测试配置
- 查看 Nginx 错误日志：`sudo tail -f /var/log/nginx/error.log`

---

**现在你可以在同一台阿里云服务器上部署后端服务了！**

需要我帮你生成具体的部署脚本吗？


