...
def _compress_profile_image(data: bytes, mime: str, max_size_kb: int = 300) -> Tuple[bytes, str]:
    """
    压缩档案照片，用于图片生成参考。
...
def _fetch_profile_image_from_oss(user_id: str, profile_id: str, photo_url: Optional[str] = None) -> Optional[Tuple[bytes, str]]:
    ...
    try:
        ...
        if len(data) >= 2 and data[0:2] == b"\xff\xd8":
            ...
            mime = "image/jpeg"
        # 压缩图片以加快 Gemini API 处理
        return _compress_profile_image(data, mime, max_size_kb=300)
    except Exception as e:
