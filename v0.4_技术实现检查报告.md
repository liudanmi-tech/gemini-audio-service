...
| 职场技能「话外音/利益保护/对话分类/局势分析/回复风格/人设」 | skills/workplace_jungle/SKILL.md（Prompt 与核心能力已按《高难度沟通·职场篇》等增强） | 符合 |
...
`skills/workplace_jungle/SKILL.md` 已按需求增强，当前能力与输出约定如下：

| 能力项 | 说明 |
|--------|------|
| 话外音识别 | 识别对方与用户话语的潜台词、真实意图与情绪，避免沟通陷阱（参考《沟通陷阱》《没人会教你的职场暗语潜规则》） |
| 利益保护 | 所有分析与策略以「最大程度保护档案中『我』的利益」为优先 |
| 对话分类 | 类型维度：冲突 / 争取 / 推进；关系维度：对上 / 对下 / 对内 / 对外（参考《高难度沟通·职场篇》） |
| 局势分析 | 权力位阶、谁占上风、风险点、用户可争取的利益点，写在策略 s0 的 content 中 |
| 关键 Topic 与回复建议 | 2～5 个关键话题，每个给出回复风格（强硬 / 阴阳怪气 / 讨好献媚 / 中性专业）及示例话术 |
| 职场人设 | 推断用户当前人设（如靠谱执行者、刺头、老好人等）及维持或微调建议 |
| 策略结构 | s0 固定为「局势与人设」总览（话外音摘要、分类、局势、关键 Topic、人设）；s1～s3 为三种破局策略，与利益保护、回复风格一致 |

后端仍只解析 `visual` 与 `strategies`，无需改 schema；上述内容均通过 `strategies[].content` 的 Markdown 输出，前端按现有逻辑渲染即可。
