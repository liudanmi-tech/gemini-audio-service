    ...
    func login(phone: String, code: String) async throws -> LoginResponse {
        ...
        if statusCode == 200 {
            guard !responseData.isEmpty else {
                throw NSError(domain: "AuthError", code: 0, userInfo: [NSLocalizedDescriptionKey: "服务器返回为空，请检查网络或稍后重试"])
            }
            do {
                ...
                throw NSError(domain: "AuthError", code: 0, userInfo: [NSLocalizedDescriptionKey: "服务器返回格式异常，请稍后重试"])
            }
        }
