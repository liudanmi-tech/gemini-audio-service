...
//  注意：使用 OnboardingCategory / OnboardingSubSkill，避免与 Skill.swift 中的 SkillCategory 冲突
//
...
struct OnboardingCategory: Identifiable, Hashable {
    ...
    let subSkills: [OnboardingSubSkill]

    func hash(into hasher: inout Hasher) { hasher.combine(id) }
    static func == (lhs: OnboardingCategory, rhs: OnboardingCategory) -> Bool { lhs.id == rhs.id }
}

struct OnboardingSubSkill: Identifiable, Hashable {
    let id: String
    ...
    func hash(into hasher: inout Hasher) { hasher.combine(id) }
    static func == (lhs: OnboardingSubSkill, rhs: OnboardingSubSkill) -> Bool { lhs.id == rhs.id }
}
...
struct SkillCategoryPresets {
    ...
    // ── 🏢 Work Life ──
    static let workLife = OnboardingCategory(
        id: "work_life", emoji: "🏢",
        name: "Work Life",
    ...
    // ── 🎓 Campus Life ──
    static let campusLife = OnboardingCategory(
    ...
    static let relationships = OnboardingCategory(
    ...
    // ── 👨‍👩‍👧 Family ──
    static let family = OnboardingCategory(
        id: "family", emoji: "👨‍👩‍👧",
        name: "Family",
    ...
    // ── 🌱 Personal Growth ──
    static let personalGrowth = OnboardingCategory(
    ...
    // ── 🏠 Life Skills ──
    static let lifeSkills = OnboardingCategory(
    ...
    static func categories(for identity: UserIdentity) -> [OnboardingCategory] {
        switch identity {
    ...
    private static func sub(_ id: String, _ cat: String, _ name: String, _ desc: String) -> OnboardingSubSkill {
        OnboardingSubSkill(id: id, categoryId: cat, name: name, description: desc)
    }
}
